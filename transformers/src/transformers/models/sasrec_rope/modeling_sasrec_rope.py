...
from .configuration_sasrec_rope import SasrecRoPEConfig
...
def apply_rope(x, seq_dim=-2):
    """
    Applies Rotary Position Embedding (RoPE) to the last two dims of x (batch, heads, seq, head_dim)
    """
...
class SasrecRoPERMSNorm(nn.Module):
...
class SasrecRoPEAttention(nn.Module):
    def __init__(self, config: SasrecRoPEConfig):
        super().__init__()
        ...
        self.norm = SasrecRoPERMSNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class SasrecRoPEBlock(nn.Module):
    def __init__(self, config: SasrecRoPEConfig):
        super().__init__()
        self.attn = SasrecRoPEAttention(config)
...
class SasrecRoPEModel(PreTrainedModel):
    config_class = SasrecRoPEConfig
    base_model_prefix = "sasrec_rope"
    supports_gradient_checkpointing = False
    _no_split_modules = ["SasrecRoPEBlock"]
    ...
    def __init__(self, config: SasrecRoPEConfig):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([SasrecRoPEBlock(config) for _ in range(config.num_hidden_layers)])
...
class SasrecRoPEForCausalLM(PreTrainedModel, GenerationMixin):
    _tied_weights_keys = {"lm_head.weight": "sasrec_rope.item_embeddings.weight"}
    supports_cache = True

    def __init__(self, config: SasrecRoPEConfig):
        super().__init__(config)
        self.sasrec_rope = SasrecRoPEModel(config)
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self.post_init()
        self.tie_weights()

    def get_input_embeddings(self) -> nn.Embedding:
        return self.sasrec_rope.get_input_embeddings()

    def set_input_embeddings(self, new_embeddings: nn.Embedding) -> None:
        self.sasrec_rope.set_input_embeddings(new_embeddings)
