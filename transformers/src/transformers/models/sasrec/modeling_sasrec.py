...
def _make_causal_mask(q_len: int, k_len: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    # allow attending to current and past positions; disallow future positions (set -inf)
...
class SasrecBlock(nn.Module):
    def __init__(self, config: SasrecConfig):
        super().__init__()
        self.attn = SasrecAttention(config)
...
class SasrecModel(SasrecPreTrainedModel):
    def __init__(self, config: SasrecConfig):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout)
        self.layers = nn.ModuleList([SasrecBlock(config) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            causal_mask = causal_mask.unsqueeze(0).unsqueeze(0)  # (1,1,Tq,Tk)
...
class SasrecForCausalLM(SasrecPreTrainedModel, GenerationMixin):
    # Tie lm_head to input embeddings to share item representations
    _tied_weights_keys = {"lm_head.weight": "sasrec.item_embeddings.weight"}
    supports_cache = True
    ...
    def get_input_embeddings(self) -> nn.Embedding:
        return self.sasrec.get_input_embeddings()

    def set_input_embeddings(self, new_embeddings: nn.Embedding) -> None:
        self.sasrec.set_input_embeddings(new_embeddings)
